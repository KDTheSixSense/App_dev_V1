...
/**
 * 基本情報A問題（PBL3基本Aデータ使用.xlsx）をデータベースに登録する
 */
async function seedBasicInfoAProblems(prisma: PrismaClient) {
  ...
  try {
    if (!fs.existsSync(filePath)) {
        console.warn(` ⚠️ File not found: ${filePath}. Skipping Basic Info A seeding.`);
        return;
    }
    ...
    if (!defaultDifficulty || !defaultSubject) {
        console.error('❌ Master data error: Default Difficulty or Subject for Basic Info A not found.');
        return;
    }
    ...
    for (const record of records) {
        ...
        const imagePath = rawImageName ? `/images/basic_a/${rawImageName}` : null;
        ...
        const existingProblem = await prisma.basic_Info_A_Question.findUnique({
            where: { id: problemId },
        });

        if (existingProblem) {
            await prisma.basic_Info_A_Question.update({ where: { id: problemId }, data: dataToSave });
            updatedCount++;
        } else {
            await prisma.basic_Info_A_Question.create({ data: { ...dataToSave, id: problemId } });
            createdCount++;
        }
    }
    console.log(`✅ Finished Basic Info A seeding. Created: ${createdCount}, Updated: ${updatedCount}.`);
...
async function main() {
  console.log(`🚀 Start seeding Basic Info A problems...`);
  await seedBasicInfoAProblems(prisma);
  console.log('✅ Seeding for Basic Info A problems finished.');
}
