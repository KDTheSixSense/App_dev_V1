...
async function GET(request) {
    ...
    const joinedEvents = allEvents.filter((event) => userJoinedEventIds.has(event.id));
    return server_1.NextResponse.json(joinedEvents);
}
/**
 * POST: イベントに参加する
 */
async function POST(request) {
    try {
        ...
        if (!eventCode) {
            return server_1.NextResponse.json({ message: "イベントコードが必要です。" }, { status: 400 });
        }
        // TODO: データベースでイベントコードに一致するイベントを検索する
        const eventToJoin = allEvents.find((event) => event.code === eventCode);
        if (!eventToJoin) {
            return server_1.NextResponse.json({ message: "無効なイベントコードです。" }, { status: 404 });
        }
        // TODO: ユーザーの参加情報をデータベースに保存する
        userJoinedEventIds.add(eventToJoin.id);
        // 参加したイベント情報を返す
        return server_1.NextResponse.json(eventToJoin, { status: 200 });
    }
    catch (error) {
