...
const useAdminData = (hashedId) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const formattedAssignments = data.data.map((assignment) => {
                    var _a, _b;
                    return (Object.assign(Object.assign({}, assignment), { programmingProblemId: (_a = assignment.programmingProblem) === null || _a === void 0 ? void 0 : _a.id, selectProblemId: (_b = assignment.selectProblem) === null || _b === void 0 ? void 0 : _b.id }));
                });
    ...
    const fetchAvailableSelectionProblems = async () => {
        ...
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                // APIからのデータに 'type' プロパティを追加して型を統一
                const typedProblems = data.map((p) => (Object.assign(Object.assign({}, p), { type: 'select' })));
    ...
    const createAssignment = async (title, description, dueDate, problem) => {
        let endpoint = '';
        let body = { assignmentTitle: title, assignmentDescription: description, dueDate };
        if (problem && 'isNew' in problem) { // 新規作成の場合
            body.problemData = Object.assign({}, problem);
