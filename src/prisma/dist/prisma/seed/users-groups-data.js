...
exports.seedUsersAndGroups = seedUsersAndGroups;
...
async function seedUsersAndGroups(prisma) {
    ...
    for (const userData of usersToSeed) {
        ...
        await prisma.user.create({
            data: Object.assign(Object.assign({ email: userData.email, username: userData.username, password: hashedPassword, level: accountLevel, xp: totalAccountXp, totallogin: getRandomInt(1, 500) }, userLoginData), { status_Kohaku: {
                    create: {
                        status: '元気',
    ...
    if (alice && bob && kobeTaro) {
        ...
        const group1 = await prisma.groups.create({
            data: {
                groupname: '神戸ゼミ',
                body: '神戸太郎が主催するゼミです。',
                invite_code: (0, nanoid_1.nanoid)(8),
            },
        });
