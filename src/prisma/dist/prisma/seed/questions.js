...
async function seedProblemsFromExcel(prisma) {
    ...
    const filePath = path_1.default.join(process.cwd(), 'app', '(main)', 'issue_list', 'basic_info_b_problem', 'data', excelFileName);
...
async function seedSelectProblemsFromExcel(prisma) {
    ...
    const filePath = path_1.default.join(__dirname, '..', '..', 'app', '(main)', 'issue_list', 'selects_problems', 'data', excelFileName);
...
function createImageFileMap() {
    ...
    const imageDir = path_1.default.join(__dirname, // 現在のディレクトリ (seed/)
    '..', // prisma/
    '..', // src/
    'public', 'images', 'basic_a');
...
function createAppliedAmImageFileMap() {
    ...
    const imageDir = path_1.default.join(process.cwd(), 'public', 'images', 'applied_am');
    // Renaming old variable to avoid matching issues with weird whitespace in subsequent lines
    const imageDir_OLD = path_1.default.join(__dirname, // 現在のディレクトリ (seed/)
    '..', // prisma/
    '..', // src/
    'public', 'images', 'applied_am' //  応用AM用のパスに変更
    );
    console.log(` 🔍 Scanning for images in: ${imageDir}`);
    const fileNameMap = new Map();
    try {
...
async function seedAppliedInfoAmProblems(prisma) {
    ...
    //  変更: 応用AM用のデータパス
    // Use process.cwd() to resolve path from project root, insensitive to dist/src location
    const filePath = path_1.default.join(process.cwd(), 'app', '(main)', 'issue_list', 'applied_info_morning_problem', 'data', excelFileName);
    try {
        const workbook = XLSX.readFile(filePath);
        const sheet = workbook.Sheets[sheetName];
        if (!sheet) {
