...
const school_festival_questions_1 = require("./school_festival_questions");
...
async function seedProblems(prisma) {
    ...
    await prisma.$executeRawUnsafe(`TRUNCATE TABLE "Submissions", "SampleCase", "TestCase", "ProblemFile", "UserAnswer", "Answer_Algorithm", "Basic_Info_A_Question", "Assignment", "Event_Issue_List", "ProgrammingProblem", "SelectProblem", "Questions", "Questions_Algorithm" RESTART IDENTITY CASCADE;`);
...
async function seedProblemsFromExcel(prisma) {
    ...
    const filePath = path_1.default.join(WORKSPACE_ROOT, 'app', '(main)', 'issue_list', 'basic_info_b_problem', 'data', excelFileName);
...
async function seedSampleProgrammingProblems(prisma, creatorId = 1) {
 ...
 */
async function seedSelectProblemsFromExcel(prisma) {
    ...
    const filePath = path_1.default.join(WORKSPACE_ROOT, 'app', '(main)', 'issue_list', 'selects_problems', 'data', excelFileName);
 ...
 */
function createImageFileMap() {
    // 1. /src/public/images/basic_a/ の絶対パスを取得
    const imageDir = path_1.default.join(WORKSPACE_ROOT, 'public', 'images', 'basic_a');
 ...
 */
function createAppliedAmImageFileMap() {
    // 1. /src/public/images/applied_am/ の絶対パスを取得
    const imageDir = path_1.default.join(
    // ✅ 修正: WORKSPACE_ROOT から 'public' を結合
    WORKSPACE_ROOT, 'public', 'images', 'applied_am');
    console.log(` 🔍 Scanning for images in: ${imageDir}`);
    const fileNameMap = new Map();
    try {
        // 2. ディレクトリ内の全ファイル名を同期的に読み込む
 ...
 * "A：... B：..." や "ア：... イ：..." に対応
 */
function parseAnswerOptionsText(text) {
 ...
 */
async function seedBasicInfoAProblems(prisma) {
    ...
    //   
    const filePath = path_1.default.join(WORKSPACE_ROOT, 'app', '(main)', 'issue_list', 'basic_info_a_problem', 'data', excelFileName);
    try {
        const workbook = XLSX.readFile(filePath);
        const sheet = workbook.Sheets[sheetName];
        if (!sheet) {
        ...
        const answerMap = { 'ア': 0, 'イ': 1, 'ウ': 2, 'エ': 3 };
        let createdCount = 0;
        let processedRowCount = 0;
        for (const record of records) {
            processedRowCount++;
            ...
            let mappedDbCategoryName = undefined;
 ...
 */
async function seedAppliedInfoAmProblems(prisma) {
    ...
    //  TODOここまで 
    //  変更: 応用AM用のデータパス
    const filePath = path_1.default.join(WORKSPACE_ROOT, 'app', '(main)', 'issue_list', 'applied_info_morning_problem', 'data', excelFileName);
    try {
        const workbook = XLSX.readFile(filePath);
        const sheet = workbook.Sheets[sheetName];
        if (!sheet) {
        ...
        const records = XLSX.utils.sheet_to_json(sheet, { header: headers, range: 2 });
        console.log(` 🔍 Found ${records.length} records in sheet "${sheetName}".`);
        if (records.length === 0) {
            console.warn(' ⚠️ No data records found.');
            return;
