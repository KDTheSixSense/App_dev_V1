...
  }
  ...
  // ★ 作成者として紐付けるユーザーを取得 (例: alice)
const problemCreator = await prisma.user.findUnique({
  where: { email: 'alice@example.com' },
});

if (!problemCreator) {
  throw new Error('Seeding failed: Could not find user "alice@example.com" to be a problem creator.');
}

// サンプル問題データ (idは自動採番されるので削除)
const problems = [
  {
    ...
    isPublic: true,
    isPublished: true,
    createdBy: problemCreator.id, // ★ 取得したユーザーのIDを紐付け
    sampleCases: {
      create: [
