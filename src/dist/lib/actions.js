...
exports.getNextProblemId = getNextProblemId;
exports.awardXpForCorrectAnswer = awardXpForCorrectAnswer;
exports.addXp = addXp;
exports.updateUserLoginStats = updateUserLoginStats;
exports.getProblemByIdAction = getProblemByIdAction;
exports.getNextProgrammingProblemId = getNextProgrammingProblemId;
exports.createGroupAction = createGroupAction;
exports.joinGroupAction = joinGroupAction;
exports.getGroupsAction = getGroupsAction;
exports.deleteProblemAction = deleteProblemAction;
exports.getMineProblems = getMineProblems;
exports.feedPetAction = feedPetAction;
exports.getMineSelectProblems = getMineSelectProblems;
exports.deleteSelectProblemAction = deleteSelectProblemAction;
exports.getSelectProblemByIdAction = getSelectProblemByIdAction;
exports.updateSelectProblemAction = updateSelectProblemAction;
...
const nanoid_1 = require("nanoid"); // 招待コード生成に使う
 ...
 */
async function registerUserAction(data) {
 ...
 */
async function getNextProblemId(currentId, category) {
    try {
        // ▼▼▼【ここから修正】▼▼▼
        ...
        if (category === 'basic_info_b_problem') {
            const [staticIds, algoIds] = await Promise.all([
                ...
                prisma_1.prisma.questions.findMany({
                    select: { id: true },
                }),
                // Questions_Algorithmテーブルから「基本情報B問題」のIDを取得
                prisma_1.prisma.questions_Algorithm.findMany({
            ...
            problemIds = await prisma_1.prisma.questions_Algorithm.findMany({
 ...
 */
async function awardXpForCorrectAnswer(problemId) {
    'use server';
    const session = await (0, iron_session_1.getIronSession)(await (0, headers_1.cookies)(), session_2.sessionOptions);
    ...
    const algoProblem = await prisma_1.prisma.questions_Algorithm.findUnique({
        where: { id: problemId },
        select: { subjectId: true, difficultyId: true },
    });
    if (algoProblem) {
        problemDetails = Object.assign(Object.assign({}, algoProblem), { type: 'ALGO' });
        // 解答履歴をチェック
        const existingAnswer = await prisma_1.prisma.answer_Algorithm.findFirst({
        ...
        const staticProblem = await prisma_1.prisma.questions.findUnique({
            where: { id: problemId },
        ...
        if (staticProblem) {
            ...
            // 解答履歴をチェック
            const existingAnswer = await prisma_1.prisma.userAnswer.findFirst({
                where: { userId, questionId: problemId, isCorrect: true },
            });
            if (existingAnswer)
                alreadyCorrect = true;
    ...
    //ユーザーの回答数を数える
    const userAnswerCount = await prisma_1.prisma.userAnswer.count({ where: { userId } });
    const algoAnswerCount = await prisma_1.prisma.answer_Algorithm.count({ where: { userId } });
    ...
    if (problemDetails.type === 'ALGO') {
        await prisma_1.prisma.answer_Algorithm.create({
            data: { userId, questionId: problemId, isCorrect: true, symbol: 'CORRECT', text: '正解' },
        });
    }
    else { // type === 'STATIC'
        await prisma_1.prisma.userAnswer.create({
    ...
    if (isFirstAnswerEver) {
        await prisma_1.prisma.status_Kohaku.updateMany({
...
async function addXp(user_id, subject_id, difficulty_id) {
    const difficulty = await prisma_1.prisma.difficulty.findUnique({
    ...
    const result = await prisma_1.prisma.$transaction(async (tx) => {
        ...
        const newSubjectLevel = (0, leveling_1.calculateLevelFromXp)(updatedProgress.xp);
        ...
        const newAccountLevel = (0, leveling_1.calculateLevelFromXp)(user.xp);
...
async function updateUserLoginStats(userId) {
    var _a, _b;
    ...
    let newConsecutiveDays = (_a = user.continuouslogin) !== null && _a !== void 0 ? _a : 0;
    let newTotalDays = (_b = user.totallogin) !== null && _b !== void 0 ? _b : 0;
...
const convertToSerializableProblem = (dbProblem) => {
    ...
    return {
        ...
        correctAnswer: ((_b = (_a = dbProblem.sampleCases) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.expectedOutput) || '',
        // ▲▲▲【修正ここまで】▲▲▲
        explanationText: {
            ja: ((_d = (_c = dbProblem.sampleCases) === null || _c === void 0 ? void 0 : _c[0]) === null || _d === void 0 ? void 0 : _d.description) || '解説は準備中です。',
            en: ((_f = (_e = dbProblem.sampleCases) === null || _e === void 0 ? void 0 : _e[0]) === null || _f === void 0 ? void 0 : _f.description) || 'Explanation is not ready yet.',
        },
 ...
 */
async function getProblemByIdAction(problemId) {
    ...
    try {
        const problemFromDb = await prisma_1.prisma.programmingProblem.findUnique({
            where: { id },
            include: {
 ...
 */
async function getNextProgrammingProblemId(currentId) {
    try {
        const nextProblem = await prisma_1.prisma.programmingProblem.findFirst({
            where: {
                id: {
                    gt: currentId
                },
...
async function createGroupAction(data) {
    ...
    const inviteCode = (0, nanoid_1.nanoid)(8);
    ...
    await prisma_1.prisma.groups_User.create({
        data: {
            user_id: userId,
...
async function joinGroupAction(inviteCode) {
    ...
    // モデル名とフィールド名をあなたのスキーマに合わせる
    const existingMembership = await prisma_1.prisma.groups_User.findUnique({
        where: {
 ...
 */
async function deleteProblemAction(formData) {
    ...
    try {
        ...
        const problem = await prisma_1.prisma.programmingProblem.findUnique({
            where: {
                id: problemId,
            },
            select: {
        ...
        await prisma_1.prisma.programmingProblem.delete({
            where: {
                id: problemId,
            },
        });
        // 5. 問題一覧ページのキャッシュをクリアし、UIを更新する
        (0, cache_1.revalidatePath)('/issue_list/mine_issue_list/problems');
...
// ★ 新しく追加する関数
async function getMineProblems() {
    'use server';
    try {
        const session = await (0, session_1.getSession)();
        const user = session.user;
        ...
        const problems = await prisma_1.prisma.programmingProblem.findMany({
 ...
 */
async function feedPetAction(difficultyId) {
    var _a;
    // 1. ログインセッションを取得
    const session = await (0, iron_session_1.getIronSession)(await (0, headers_1.cookies)(), session_2.sessionOptions);
    if (!((_a = session.user) === null || _a === void 0 ? void 0 : _a.id)) {
    ...
    try {
        ...
        if (!petStatus) {
            ...
            await prisma_1.prisma.status_Kohaku.create({
                data: {
                    user_id: userId,
                    status: "満腹",
            ...
            await prisma_1.prisma.status_Kohaku.update({
                where: {
                    id: petStatus.id,
                },
                data: {
 ...
 */
async function getMineSelectProblems() {
    'use server';
    try {
        const session = await (0, session_1.getSession)();
        const user = session.user;
        ...
        const problems = await prisma_1.prisma.selectProblem.findMany({
            where: { createdBy: userId },
            include: {
                creator: {
                    select: { username: true },
 ...
 * @param formData - フォームから送信されたデータ
 */
async function deleteSelectProblemAction(formData) {
    'use server';
    const session = await (0, session_1.getSession)();
    const user = session.user;
    if (!(user === null || user === void 0 ? void 0 : user.id)) {
    ...
    try {
        ...
        await prisma_1.prisma.selectProblem.delete({
            where: { id: problemId },
        });
        // キャッシュをクリアしてUIを更新
        (0, cache_1.revalidatePath)('/issue_list/mine_issue_list/problems');
 ...
 */
async function getSelectProblemByIdAction(problemId) {
    'use server';
    try {
        const session = await (0, session_1.getSession)();
        if (!session.user) {
 ...
 * @param formData フォームから送信されたデータ
 */
async function updateSelectProblemAction(formData) {
    'use server';
    try {
        const session = await (0, session_1.getSession)();
        const user = session.user;
        ...
        const answerOptions = JSON.parse(formData.get('answerOptions'));
