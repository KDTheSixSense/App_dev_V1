...
const binarySearchLogic: { traceLogic: TraceStep[]; calculateNextLine: (currentLine: number, vars: VariablesState) => number } = {
    traceLogic: Array(16).fill((vars: VariablesState) => vars), // Dummy array
    calculateNextLine(currentLine, vars) {
        // 初期化
        if (!vars.initialized) {
            const data = vars.data as number[];
            vars.low = 1;
            vars.high = data.length;
            vars.middle = null;
            vars.result = null;
            vars.initialized = true;
        }
        ...
        switch (lineNum) {
            case 4: // low ← 1
                return 4; // -> 5行目へ
            case 5: // high ← dataの要素数
                return 5; // -> 6行目へ
            case 6: // while (low <= high)
                return low <= high ? 6 : 15; // -> 7行目 or 16行目
            case 7: // middle ← (low + high) ÷ 2 の商
                vars.middle = Math.floor((low + high) / 2);
                return 7; // -> 8行目へ
            case 8: // if (data[middle] < target)
                // 配列アクセスは0-indexed
                if (data[vars.middle - 1] < target) {
                    return 8; // -> 9行目へ
                } else if (data[vars.middle - 1] > target) {
                    return 10; // -> 11行目へ
                } else {
                    return 12; // -> 13行目へ
                }
            case 9: // low ← middle
                vars.low = vars.middle;
                return 5; // ループの先頭(6行目)に戻る
            case 11: // high ← middle
                vars.high = vars.middle;
                return 5; // ループの先頭(6行目)に戻る
