...
export default function CategoryFilter() {
  ...
  useEffect(() => {
    const savedCategory = localStorage.getItem('applied_info_morning_filter_category');
    const savedStatus = localStorage.getItem('applied_info_morning_filter_status');

    const params = new URLSearchParams(searchParams.toString());
    let shouldUpdate = false;

    if (!searchParams.has('category') && savedCategory && savedCategory !== 'all') {
  ...
  const updateParams = (key: string, value: string) => {
    localStorage.setItem(`applied_info_morning_filter_${key}`, value);
    const params = new URLSearchParams(searchParams.toString());
    if (value === 'all') {
      params.delete(key);
    } else {
  ...
  return (
    <div className="mb-6 flex justify-end max-w-6xl mx-auto gap-4">
