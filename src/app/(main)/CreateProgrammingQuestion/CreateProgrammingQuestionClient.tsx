...
interface CreateProgrammingQuestionClientProps {
  initialData: InitialProblemData | null;
}

// プログラミング問題作成ページのメインコンポーネント（FOUC対策版）
export default function CreateProgrammingQuestionClient({
  initialData,
}: CreateProgrammingQuestionClientProps) {
  // フォームの状態管理
  ...
  const [problemId, setProblemId] = useState<number | null>(initialData?.problemId || null);
  const [activeTab, setActiveTab] = useState("basic"); // アクティブなタブ
  const [selectedCategory, setSelectedCategory] = useState(
    initialData?.selectedCategory || "programming"
  ); // 選択されたカテゴリ
  ...
  // 編集モード時にデータを取得するuseEffect -> initialDataで置き換え
  useEffect(() => {
    // initialData を既に持っているので、初回フェッチは不要
    // 必要に応じて、router.refresh() を使用することもできる
  }, [initialData]);
  ...
  const addTestCase = () => {
    ...
    setTestCases((prev) => [
      ...prev,
      { id: newId, name: `ケース${testCases.length + 1}`, input: "", expectedOutput: "", description: "" },
    ]);
  };
    ...
    try {
      ...
      if (selectedCategory === "itpassport") {
        const requestBody = {
          ...
          correctAnswer:
            answerOptions.find((opt) => opt.id === correctAnswer)?.text || "",
  ...
  const handlePublishProblem = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (selectedCategory === "itpassport") {
        ...
        if (!response.ok)
          throw new Error(
            (await response.json()).message || "選択問題の作成に失敗しました。"
          );
        await response.json();
        ...
        if (!response.ok)
          throw new Error(
            (await response.json()).error || "コーディング問題の投稿に失敗しました"
          );
        await response.json();
  ...
  // 選択問題用のタブ（設定タブは非表示に）
  const selectProblemTabs = [{ id: "basic", label: "基本情報" }];
  const tabsToRender =
    selectedCategory === "itpassport" ? selectProblemTabs : programmingTabs;
      ...
      <div className="main-layout">
        ...
        <div className="sidebar">
          <div className="sidebar-header">
            <div className="sidebar-title">問題作成カテゴリ</div>
            {isEditMode && <div className="edit-mode-indicator">編集モード</div>}
          </div>
              ...
              {selectedCategory === "itpassport" ? (
                      ...
                      <div className="card-body">
                        ...
                        <div className="form-group">
                          <label className="form-label">
                            <span className="required-badge">必須</span>問題タイトル
                          </label>
                          <input
                            type="text"
                            ...
                            placeholder="例: Pythonの変数宣言について"
                            required
                          />
                        </div>
                        <div className="form-group">
                          <label className="form-label">
                            <span className="required-badge">必須</span>問題文
                          </label>
                          <textarea
                        ...
                        <div className="form-group">
                          <label className="form-label">
                            <span className="required-badge">必須</span>選択肢と正解
                          </label>
                ...
                <>
                  ...
                  {activeTab === "description" && (
                    <div className="card">
                      <div className="card-header">問題文作成</div>
                      <div className="card-body">
                        <div className="form-group">
                          <label className="form-label">
                            <span className="required-badge">必須</span>
                  ...
                  {activeTab === "sample-cases" && (
                    <div className="card">
                      <div className="card-header">サンプルケース管理</div>
                      <div className="card-body">
                        <div className="form-group">
                          <div
                            style={{
                          ...
                          {sampleCases.map((sampleCase, index) => (
                            <div
                              key={sampleCase.id ?? `new-sample-${index}`}
                              className="case-item"
                            >
                                  ...
                                  <textarea
                                    ...
                                    onChange={(e) => {
                                      setSampleCases((prev) =>
                                        prev.map((c) =>
                                          c.id === sampleCase.id
                                            ? { ...c, expectedOutput: e.target.value }
                                            : c
                                        )
                                      );
                                    }}
                  ...
                  {activeTab === "test-cases" && (
                    <div className="card">
                      <div className="card-header">テストケース管理</div>
                      <div className="card-body">
                        <div className="form-group">
                          <div
                            style={{
                          ...
                          {testCases.map((testCase, index) => (
                            <div
                              key={testCase.id ?? `new-test-${index}`}
                              className="case-item"
                            >
                              <div className="case-header">
                                <div className="case-title">
