...
interface MemberGroupClientPageProps {
  initialGroup: GroupDetail | null;
  initialPosts: Post[];
  initialKadaiList: Kadai[];
  initialHasMorePosts: boolean;
  initialHasMoreKadai: boolean;
}

const MemberGroupClientPage: React.FC<MemberGroupClientPageProps> = ({
  initialGroup,
  initialPosts,
  initialKadaiList,
  initialHasMorePosts,
  ...
  const [kadaiList, setKadaiList] = useState<Kadai[]>(initialKadaiList);
  const [loading, setLoading] = useState(false); // 初期データは既にロード済み
  ...
  const [hasMoreKadai, setHasMoreKadai] = useState(initialHasMoreKadai);
  const [loadingMoreKadai, setLoadingMoreKadai] = useState(false);

  // 課題表示関連
  const [selectedKadai, setSelectedKadai] = useState<Kadai | null>(null);
  const [kadaiViewMode, setKadaiViewMode] = useState<"list" | "detail">
    ("list");

  // === データ取得 ===
  // useEffectは初回ロードではなく、以降のページネーションでのみ使用
  ...
  const handleLoadMorePosts = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/groups/${hashedId}/posts?page=${nextPage}&limit=20`
      );
  ...
  const handleLoadMoreKadai = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/groups/${hashedId}/assignments?page=${nextPage}&limit=20`
      );
  ...
  const handleAssignmentSubmit = (kadaiId: number) => {
    ...
    setKadaiList((prev) =>
      prev.map((k) => (k.id === kadaiId ? { ...k, completed: true } : k))
    );
    // 詳細画面を閉じて一覧に戻る
  ...
  return (
    <GroupLayout>
      ...
      {error && (
        <div style={{ padding: "2rem", color: "red" }}>エラー: {error}</div>
      )}

      {!loading && !group && (
        <div style={{ padding: "2rem" }}>グループが見つかりません。</div>
      )}
              ...
              <div>
                {kadaiViewMode === "list" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center" }}>
                            <div
                              style={{
                                ...
                                height: "32px",
                                borderRadius: "50%",
                                backgroundColor: kadai.completed
                                  ? "#4caf50"
                                  : "#d32f2f",
                                display: "flex",
                                alignItems: "center",
                            ...
                            <div>
                              ...
                              <p style={{ fontSize: "14px", color: "#5f6368", margin: 0 }}>
                                期限:
                                {kadai.dueDate
                                  ? new Date(kadai.dueDate).toLocaleString(
                                      "ja-JP"
                                    )
                                  : "未設定"}
                              </p>
                    ...
                    {hasMoreKadai && (
                      <button
                        onClick={handleLoadMoreKadai}
                        disabled={loadingMoreKadai}
                        style={{
                          marginTop: "16px",
                          padding: "10px 16px",
                          border: "1px solid #00bcd4",
...
export default MemberGroupClientPage;
